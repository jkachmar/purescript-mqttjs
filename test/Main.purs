...
--------------------------------------------------------------------------------
-- Alias for a record of AVars that we'll use to ferry information between the
...
type TestEffects e =
  (avar :: AVAR, buffer :: BUFFER, mqtt :: MQTT | e)

--------------------------------------------------------------------------------
-- The url of some MQTT broker we'll test against.
url :: BrokerUrl
...
--------------------------------------------------------------------------------
-- A coroutine `Consumer` that updates the relevant fields within `TestFerry`
-- whenever events are received from the MQTT client.
mqttConsumer :: ∀ e. TestFerry -> MQTTConsumer (TestEffects e) Unit
    ...
    describe "purescript-mqttjs" do
      describe "Roundtrip" do
        it "connects to the MQTT broker" do
          ...
          putVar {client, fiber, ferry} scaffold
        ...
        it "disconnects from the MQTT broker" do
            {client, fiber, ferry} <- readVar scaffold
